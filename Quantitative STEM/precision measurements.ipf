...
// 1/5/15: added SegmentedPeakPositions to split up peak finding into subregions in order to work better for experiemntal images with 
// thickness non-uniformity. ABY.
// 1/5/15: added  IntegratedGaussianIntensity. Integrates atom column intensities, calculates atom column visibilities compared to their 
// neighbors, and creates image maps of both so the user can see atom columns of interest. ABY.
// 1/14/15: Addeded function ZeroBackgroundImage. ABY
// 1/14/15: Updated SegmentedPeakPositions to find the maximumum pixel intensity for the initial guess of the atom positions.
// 1/14/15: Found a possible bug in wiggle code. I think it may fail the whole fit if one of the wiggle fits fail. If one of the wiggle fits work correctly it should be kept.


// Sets all pixels with intensity less than threshold to zero. Outputs image_zeroed.
function ZeroBackgroundImage(image, threshold)
	
	wave image
	variable threshold
	Duplicate/O image image_zeroed
...
// This function finds the peak positions using the analyze particle IGOR function and reports the locations in x_loc and y_loc.
// For Si dumbells, it reports the center of the dumbells. To change the mimimun area of the particles change the A flag in the ImageAnalyzeParticle function.
// Inputs: image, window_size is the size of the smaller segment image used in the particle find. It is recommended to use a window size that is larger than the crystal repeat distance.
function SegmentedPeakPositions(image, window_size)
	
	wave image
	variable window_size
	
	variable size_x = dimsize(image, 0)
	variable size_y = dimsize(image, 1)
	variable delta = window_size/4
	variable numsteps_x = round((size_x/(delta))-1)
	variable numsteps_y = round((size_y/(delta))-1)
	variable initial_x, initial_y, final_x, final_y
	make/o/n=0 y_loc, x_loc
	
	variable i, j, k, n = 0
	for(i=0; i<numsteps_x; i+=1)
		for(j=0; j<numsteps_y; j+=1)
			//calculate and crop the sub area to analyze
			initial_x = i * delta
			initial_y = j * delta
			if (i == (numsteps_x - 1))
				final_x = dimsize(image,0)
			else
				final_x = initial_x+window_size
			endif
			if (j == (numsteps_y - 1))
				final_y = dimsize(image,1)
			else
				final_y = initial_y+window_size
			endif
			duplicate/o image image_tmp
			cropimage(image_tmp, initial_x, initial_y,  final_x, final_y)
			
			// Add ROI to ignore pixels in input image that are NaN
			duplicate/O image_tmp roi_tmp
			Redimension/b/u roi_tmp
			roi_tmp = numtype(image)
			
			//Manual Threshold based of certain percent of intensity range. hard coded for 66%
			wavestats/q image_tmp
			variable thresh = ((V_max - V_min)*0.66) + V_min
			ImageThreshold/I/M=(0)/Q/R={roi_tmp, 2}/T=(thresh) image_tmp
			ImageAnalyzeParticles /E/W/Q/F/M=3/A=1/EBPC stats, M_ImageThresh
			
			//Auto Threshold
			//ImageThreshold/I/M=(1)/Q/R={roi_tmp, 2} image_tmp
			//ImageAnalyzeParticles /E/W/Q/F/M=3/A=5/EBPC stats, M_ImageThresh
			
			//calculate and save positions for found atoms
			duplicate/O W_xmin x_loc_tmp
			duplicate/O W_ymin y_loc_tmp
			wave W_xmin = $"W_xmin"
			wave W_ymin = $"W_ymin"
			wave W_xmax = $"W_xmax"
			wave W_ymax = $"W_ymax"
			
			// procedure to use center position in found particle
			//x_loc_tmp = ((W_xmax + W_xmin) / 2) + initial_x
			//y_loc_tmp = ((W_ymax + W_ymin) / 2) + initial_y
			
			// procedure to find max intensity pixel of particle. Added 1/14/15 ABY
			for (n = 0; n < dimsize(x_loc_tmp,0); n+=1)
				variable xMax = W_xmin[n]
				variable yMax = W_ymin[n]
				variable tmpMax = image_tmp[W_xmin[n]][W_ymin[n]]
				variable f = W_xmin[n]
				do
					variable g = W_ymin[n]
					do
						if (tmpMax < image_tmp[f][g])
							tmpMax = image_tmp[f][g]
							xMax = f
							yMax = g
						endif
						g += 1
				...
				x_loc_tmp[n] = xMax + initial_x
				y_loc_tmp[n] = yMax + initial_y
			endfor
			
			//insert points into x_loc and y_loc
			for(k=0; k<dimsize(x_loc_tmp,0); k+=1)
				InsertPoints 0, 1, x_loc
				x_loc[x2pnt(x_loc,0)] = x_loc_tmp[k]
				InsertPoints 0, 1, y_loc
				y_loc[x2pnt(y_loc,0)] = y_loc_tmp[k]
			endfor
			
			//save an example area which is the first sub-area analyzed
			if ( i == (0) && j == (0))
				duplicate/o image_tmp example_segment
			endif
			
			killwaves image_tmp, x_loc_tmp, y_loc_tmp
		endfor	
	endfor
	
	//Sort through x_loc and y_loc to get rid of the same atom columns
	variable d
	variable l, m = 0
	for(l=0; l<dimsize(x_loc,0); l+=1)
		for(m=l+1; m<dimsize(x_loc,0); m+=1)
			d = sqrt(((x_loc[l] - x_loc[m])^2) + ((y_loc[l] - y_loc[m])^2))
			if(d<4)
				deletepoints m, 1, x_loc
				deletepoints m, 1, y_loc
				m = m - 1
			endif
		endfor
	endfor
	
	//Display image with found positions
	NewImage image
	appendtograph/t y_loc vs x_loc
	...
	Killwaves roi_tmp
	Killwaves M_ImageThresh, M_Particle
end
...
function GaussianFit(image, x_loc, y_loc, size, wiggle, [fix_xy, no_noise])
	...
	for(i=0; i<num_peaks; i+=1)
		...
		success = 1
		// fit with held x and y center positions
		if(fix_xy == 1)
			// Use no wiggle
			if(wiggle == 0)
				// leave out Poisson noise weighting from the fit
				if(no_noise == 1)
					V_FitError = 0
				...
				// Includes Poisson noise weighting from the fit
				else
					V_FitError = 0
		...
		// fit without held x and y center positions
		else
			// leave out Poisson noise weighting from the fit
			if(no_noise == 1)
				// Use no wiggle
				if(wiggle == 0)
					V_FitError =0
					CurveFit/M=2/W=2/Q gauss2D, image[x_start[i],x_finish[i]][y_start[i],y_finish[i]] /I=1 /D
					if(V_FitError)
						success = 0
						printf "V_FitError = %d.\t", V_FitError
					endif
	...
	killwaves W_sigma, W_coef, x_finish, x_start, y_finish, y_start, noise
end
...
function OneGaussFitWiggle(image, x_loc, y_loc, size, wiggle)
	...
	// I think wiggle is bugged. I think it may fail an atom fit if any of the wiggle fits fail. It should not fail the fit if one of the wiggle fits actually work.
	for(i=-1.0*wiggle; i<=wiggle; i+=1)
		for(j=-1.0*wiggle; j<=wiggle; j+=1)
			...
			//hardcoded for no noise
			CurveFit/N/NTHR=0/W=2/Q gauss2D, im/R=residual
...
// This only works for 2D Gaussian fits. Si dumbbells need a different fitting routine.
// Inputs: image, x_loc, y_loc, and gaussian fit size x size.
function OldGaussianFit(image, x_loc, y_loc, size, wiggle, no_noise)
	
	wave image, x_loc, y_loc
	variable size, wiggle, no_noise
	variable half_size = size / 2
	variable num_peaks = DimSize(x_loc,0)
	...
	Make/O/N=(num_peaks) cor, A, z0, yW, xW, y0, x0
	Make/O/N=(num_peaks) sigma_cor, sigma_A, sigma_z0, sigma_yW, sigma_xW, sigma_y0, sigma_x0
	...
	for(i=0; i<num_peaks; i+=1)
		...
		if(wiggle == 0)
			if (no_noise == 0)
				CurveFit/N/M=2/W=2/Q gauss2D, image[x_start[i],x_finish[i]][y_start[i],y_finish[i]] /W=noise /I=1 /D/R
			else
				CurveFit/N/M=2/W=2/Q gauss2D, image[x_start[i],x_finish[i]][y_start[i],y_finish[i]]/D/R
			endif
		else	
			OldOneGaussFitWiggle(image, x_loc[i], y_loc[i], size, wiggle, no_noise)
		endif
...
// The fit with the minimum chisq is considered the best, and the coefficients are maintained
// in waves W_Coef and W_Sigma. All other fitting coefficients are discarded.
function OldOneGaussFitWiggle(image, x_loc, y_loc, size, wiggle, no_noise)
	
	wave image
	variable x_loc, y_loc, size, wiggle, no_noise
	...
	for(i=-1.0*wiggle; i<=wiggle; i+=1)
		for(j=-1.0*wiggle; j<=wiggle; j+=1)
			...
			if (no_noise == 0)
				...
				CurveFit/N/M=2/W=2/Q gauss2D,image[x_start,x_finish][y_start,y_finish]/D/R=single_residual
			endif

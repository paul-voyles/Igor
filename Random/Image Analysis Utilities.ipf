...
// CombineImages			combines 2 images, replacing the NaN pixels in im1 with the values in im2.
//
// PMV and WGS, 6/25/03.
//
// renamed from "FEM Analysis Utilities.ipf", 8/12/14.  Not compatible with "STEM" group of procedures.
// added CombineImages, 1/12/15. ABY
...
// combines two images, such that all the pixels in im1 that have NaN are replaced by the values for that pixel in im2.
// All other pixels in im1 are left alone.
function CombineImages(im1, im2)
	wave im1, im2
	variable value
	
	Duplicate/o im1, combine
	
	variable i, j
	for(i=0; i<DimSize(im1, 0); i+=1)
		for(j=0; j<DimSize(im1, 1); j+=1) 
			if(numtype(im1[i][j]) == 2)
				combine[i][j] = im2[i][j]
			endif
		endfor
	endfor

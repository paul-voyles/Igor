...
function GaussianFit(image, x_loc_s, y_loc_s, size_s)
	...
	x_start = x_loc_s - round(size_s/2)
	x_finish = x_loc_s + round(size_s/2)
	y_start = y_loc_s - round(size_s/2)
	y_finish = y_loc_s + round(size_s/2)
	...
	for(i=0; i<num_peaks_s; i+=1)
		...
		cropimage(im, x_start(i), y_start(i), x_finish(i)-1, y_finish(i)-1)
		if( i == 0 )
			Make/O/N=(Dimsize(im,0), Dimsize(im,1), (num_peaks_s)) im_s
		endif
		...
		if (V_FitError !=0)
			print "error in single atom fit - slice #", i, "- V_FitError =", V_FitError
			z0_s[i] = NaN
			A_s[i] = NaN
			x0_s[i] = NaN
			xW_s[i] = NaN
...
function DumbbellGaussianFit(image, x_loc_d, y_loc_d, size_d_x, size_d_y)
	...
	for(i=0; i<DimSize(x_loc_d,0); i+=2)
		x_start[i/2] = round(((x_loc_d(i) + x_loc_d(i+1))/2)) - round(size_d_x/2)
		x_finish[i/2] = round(((x_loc_d(i) + x_loc_d(i+1))/2)) + round(size_d_x/2)
		y_start[i/2] = round(((y_loc_d(i) + y_loc_d(i+1))/2)) - round(size_d_y/2)
		y_finish[i/2] = round(((y_loc_d(i) + y_loc_d(i+1))/2)) + round(size_d_y/2)
	endfor	
	...
	variable xw1s = size_d_x/6, xw2s = size_d_x/6, yw1s = size_d_x/6, yw2s = size_d_x/6
	
	i=0
	for(i=0; i<num_peaks_d; i+=2)
		//crop out, save, and setscale of area to be fitted
		Duplicate/O image im
		cropimage(im, x_start(i/2), y_start(i/2), x_finish(i/2)-1, y_finish(i/2)-1)
		if( i == 0 )
			Make/O/N=(Dimsize(im,0), Dimsize(im,1), (num_peaks_d/2)) im_d
		endif
		imagetransform /INSI=im /INSX=0 /INSY=0 /P=(i/2) insertImage im_d
		setscale/P x, x_start(i/2), 1, im
		setscale/P y, y_start(i/2), 1, im
		...
		if (V_FitError !=0)
			print "error in dumbbell atom fit - slice # =", i, "- V_FitError =", V_FitError
			z0_d[i] = NaN
			A_d[i] = NaN
			x0_d[i] = NaN
			xW_d[i] = NaN

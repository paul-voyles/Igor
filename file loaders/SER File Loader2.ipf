...
function LoadSERFolder()
	...
	do
		...
		// Added to sequentially number loaded ser files from folder (aby 6_19_14)
		string num
		sprintf num, "im_%d", i
		ser_name = num
...
function LoadNewSER(file)
	...
	//print "dimension", dimension
 	...
 	//print "nsets", nsets
 	//print "offset_array_pos", offset_array_pos
 	//print "dtime", dtime	
	...
	// Read the data 	
 	...
 	elseif (dimension == 2 && nsets > 1)
 		for(iset = 0; iset<nsets; iset+=1)
 			FSetPos fnum, offset_array_pos + 8*iset
 			FbinRead/F=3 fnum, cpos
			if( iset == (nsets-1))
				FStatus fnum
				tag_offset_array_pos = V_FilePos
	...
	elseif (nsets > 1)
		...
		for(iset=0; iset<nsets; iset+=1)
		 	FSetPos fnum, tag_offset_array_pos + 8*iset
 			FBinRead/F=3 fnum, cpos
 			FSetPos fnum, cpos
	 		FBinRead/f=3 fnum, tmp
 			FBinRead/f=3 fnum, dtime	
